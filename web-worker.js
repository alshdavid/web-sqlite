...
globalThis.onmessage = async ({ data: clientData }) => {
  ...
  try {
    if (t === MessageType.Open) {
      ...
      globalThis.postMessage([ref, true, undefined]);
    }
    //
      ...
      if (!db) {
        globalThis.postMessage([ref, undefined, "Database not found"]);
        return;
      }
      const result = [];
      ...
      globalThis.postMessage([ref, result, undefined]);
    }
  } catch (error) {
    globalThis.postMessage([ref, undefined, error.toString()]);
    console.error(error);
  }
};
